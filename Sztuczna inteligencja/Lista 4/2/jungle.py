...
class Jungle:
    ...
    def copy(self):
        newGame = Jungle()
        newGame.board = [i[:] for i in self.board]
        newGame.pieces[0] = self.pieces[0].copy()
        newGame.pieces[1] = self.pieces[1].copy()

        newGame.curplayer = self.curplayer
        newGame.winner = self.winner
        newGame.peace_counter = self.peace_counter
