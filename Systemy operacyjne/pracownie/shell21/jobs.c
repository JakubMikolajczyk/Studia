...
static void sigchld_handler(int sig) {
  ...
  if (WIFEXITED(status)) { // natural end
  ...
  for (int j = 0; j < njobmax; j++) {
    ...
    for (int p = 0; p < jobs[j].nproc; p++) {
      if (jobs[j].proc[p].pid == pid) {
        if (!WIFEXITED(status)) // in natural end job is FINISHED when all proc
                                // are FINISHED
          jobs[j].state = state;
...
bool resumejob(int j, int bg, sigset_t *mask) {
  ...
  if (!bg) {
    ...
    tcsetattr(tty_fd, TCSANOW, &jobs[0].tmodes);
    Kill(-jobs[0].pgid, SIGCONT);
    sigsuspend(mask);
    printf("continue \'%s\', status=%d\n", jobcmd(0), exitcode(&jobs[0]));
    monitorjob(mask);
  } else {
...
void watchjobs(int which) {
  for (int j = BG; j < njobmax; j++) {
    ...
    if (job->state != which && which != ALL)
      continue;

    if (job->state == FINISHED && exitcode(job) < 2)
      printf("[%d] exited \'%s\', status=%d\n", j, jobcmd(j), exitcode(job));

    if (job->state == FINISHED && exitcode(job) > 2)
      printf("[%d] killed \'%s\' by signal %d\n", j, jobcmd(j), exitcode(job));

    if (job->state == RUNNING)
      printf("[%d] running \'%s\', status=%d\n", j, jobcmd(j), exitcode(job));

    if (job->state == STOPPED && exitcode(job) == SIGTSTP)
      printf("[%d] stopped \'%s\',  status=%d\n", j, jobcmd(j), exitcode(job));

    if (job->state == STOPPED &&
        (exitcode(job) == SIGTTIN || exitcode(job) == SIGTTOU))
      printf("[%d] suspended \'%s\',  status=%d\n", j, jobcmd(j),
             exitcode(job));
