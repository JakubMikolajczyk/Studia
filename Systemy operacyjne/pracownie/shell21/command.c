...
noreturn void external_command(char **argv) {
  ...
  if (!index(argv[0], '/') && path) {
    ...
    size_t i = strcspn(path + 0, ":");
    size_t prevI = 0;
    while (prevI < n) {
      // printf("I: %d, prevI: %d, restPath ",i, prevI, path + prevI);
      char *newPath = strndup(path + prevI, i);
      strapp(&newPath, "/");
      strapp(&newPath, argv[0]);

      // printf(" PATH: %s\n",newPath);
      prevI += i + 1;
      i = strcspn(path + prevI + 1, ":") + 1;
      (void)execve(newPath, argv, environ);
    }
    exit(EXIT_FAILURE);
#endif /* !STUDENT */
  } else {
