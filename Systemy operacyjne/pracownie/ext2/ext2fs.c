 ...
 */
#ifdef STUDENT
/* Imię i nazwisko, numer indeksu: Jakub Mikolajczyk, 324504 */
#endif
...
int ext2_block_used(uint32_t blkaddr) {
  ...
  uint32_t blk_grp_id = (blkaddr - 1) / blocks_per_group;
  blk_t *blk = blk_get(0, group_desc[blk_grp_id].gd_b_bitmap);
  uint32_t loc_grp_id = (blkaddr - 1) % blocks_per_group;

  used = *((int *)(blk->b_data) + (loc_grp_id / 32));
  used = (used >> (loc_grp_id % 32)) & 1;
  blk_put(blk);
...
int ext2_inode_used(uint32_t ino) {
  ...
  uint32_t blk_grp_id = (ino - 1) / inodes_per_group;
  blk_t *blk = blk_get(0, group_desc[blk_grp_id].gd_i_bitmap);
  uint32_t loc_grp_id = (ino - 1) % inodes_per_group;
...
static int ext2_inode_read(off_t ino, ext2_inode_t *inode) {
  ...
  size_t offset = loc_grp_id * sizeof(ext2_inode_t);
  size_t pos = BLKSIZE * group_desc[blk_grp_id].gd_i_tables + offset;

  return ext2_read(0, inode, pos, sizeof(ext2_inode_t));
...
static uint32_t ext2_blkptr_read(uint32_t blkaddr, uint32_t blkidx) {
  ...
  uint32_t blk_ptr = ((uint32_t *)blk->b_data)[blkidx];
  blk_put(blk);
  return blk_ptr;
...
long ext2_blkaddr_read(uint32_t ino, uint32_t blkidx) {
  ...
  if (blkidx < EXT2_NDADDR) // direct
    return inode.i_blocks[blkidx];

  blkidx -= EXT2_NDADDR;
  if (blkidx < BLK_POINTERS) // single
    return ext2_blkptr_read(inode.i_blocks[EXT2_NDADDR], blkidx);

  blkidx -= BLK_POINTERS;

  if (blkidx < BLK_POINTERS * BLK_POINTERS) { // double
    uint32_t fst_blk_idx = blkidx / BLK_POINTERS;
    uint32_t scd_blk_adr =
      ext2_blkptr_read(inode.i_blocks[EXT2_NDADDR + 1], fst_blk_idx);

    uint32_t scd_blk_idx = blkidx % BLK_POINTERS;
    return ext2_blkptr_read(scd_blk_adr, scd_blk_idx);
  }

  blkidx -= (BLK_POINTERS * BLK_POINTERS);

  if (blkidx < (BLK_POINTERS * BLK_POINTERS * BLK_POINTERS)) { // triple
    uint32_t fst_blk_idx = blkidx / (BLK_POINTERS * BLK_POINTERS);
    uint32_t scd_blk_adr =
      ext2_blkptr_read(inode.i_blocks[EXT2_NDADDR + 2], fst_blk_idx);

    uint32_t scd_blk_idx = blkidx / BLK_POINTERS;
    uint32_t trd_blk_adr = ext2_blkptr_read(scd_blk_adr, scd_blk_idx);

    uint32_t trd_blk_idx = blkidx % BLK_POINTERS;
    return ext2_blkptr_read(trd_blk_adr, trd_blk_idx);
  }
...
int ext2_read(uint32_t ino, void *data, size_t pos, size_t len) {
  ...
  while (len > 0) {
    uint32_t blk_offset = pos % BLKSIZE;
    uint32_t blk_bytes_to_end = BLKSIZE - blk_offset;

    uint32_t to_read;

    if (len < blk_bytes_to_end)
      to_read = len;
    else // read full block to end
      to_read = blk_bytes_to_end;

    uint32_t blk_idx = pos / BLKSIZE;
    blk = blk_get(ino, blk_idx);

    if (blk == BLK_ZERO)
      memset(data, 0, to_read);
    else {
      memcpy(data, blk->b_data + blk_offset, to_read);
      blk_put(blk);
    }
...
int ext2_readlink(uint32_t ino, char *buf, size_t buflen) {
  ...
  return ext2_read(ino, buf, 0, buflen);
...
int ext2_lookup(uint32_t ino, const char *name, uint32_t *ino_p,
  ...
  while (ext2_readdir(ino, &off_p, &dir)) {
    *ino_p = dir.de_ino;
    *type_p = dir.de_type;

    if (strcmp(dir.de_name, name))
      return 0;
  }
...
int ext2_mount(const char *fspath) {
  ...
  group_desc_count =
    block_count / blocks_per_group + (block_count % blocks_per_group != 0);
