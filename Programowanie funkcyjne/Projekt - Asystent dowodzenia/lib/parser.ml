...
(*======================================================================================================================================================================================================*)
(* NOT MY CODE *)
(* FROM https://github.com/pyrocat101/opal *)
...
let p_impl = token "->" >> return (fun p q -> IMP(p, q))
let p_and = token "^" >> return (fun p q -> AND(p, q))
let p_or = token "V" >> return (fun p q -> OR(p, q))
let p_not p = token "not" >> p => (fun x -> NOT(x))
let p_var = spaces >> many1 letter => implode % (fun x -> VAR  x)
let p_bot = exactly 'F' >> return False
let p_top = exactly 'T' >> return True

let rec expr input = (chainr1 and_expr p_impl) input
and and_expr input = (chainl1 or_expr p_and) input
and or_expr input = (chainl1 prm_expr p_or) input
and prm_expr input = (p_not (expr) <|> parens expr <|> p_var <|> p_top  <|> p_bot) input



let command str = parse
  (choice [
    token "intro" >> spaces >> many1 alpha_num => implode % (fun name -> IntroByName(name));
    token "intro" >> spaces >> return Intro;
    token "apply" >> spaces >> return Apply;
    token "qed"   >> spaces >> return QED;
    token "goal"   >> spaces >> return Goal;
    token "left"   >> spaces >> return Left;
    token "right"   >> spaces >> return Right;
    token "elimDNot"   >> spaces >> return ElimDNot;
    token "elimByName"  >> spaces >> many1 alpha_num => implode % (fun name -> ElimByName(name));
    token "elim"   >> spaces >> expr => (fun x -> Elim(x));
    token "proof"   >> spaces >> expr => (fun x -> Proof(x));
    token "next"  >>  spaces >> return Next
  ]) (LazyStream.of_string str)

  let parse_formula str = 
    match parse expr (LazyStream.of_string str) with
      | Some ans -> ans
      | None -> failwith "Wrong formula"
